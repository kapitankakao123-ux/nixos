...
- рабочее место, пока `deskjet` и `rpinix` переустанавливаются
...
- [x] `networking.hostId = "ab1bac40"` — заполнен при включении ZFS, см. [[ZFS-хранилище]]
...
Пошаговый разбор с объяснениями — [[ZFS-хранилище]]. Здесь только статус.
...
- [x] Автологин в niri без ввода пароля — включён 2026-09-14, см. «Автовход» ниже

## Автовход (2026-09-14)

Машина стоит у телевизора, поэтому после перезагрузки должна сама оказаться
в сессии. Сделано через greetd, в `hosts/kitjet/default.nix`:
...
Как это устроено: у greetd **две** половины настройки, и они не
взаимоисключающие.

| | что это | где задано |
|---|---|---|
| `default_session` | греетер, который спрашивает, кем войти | `modules/desktop.nix` — общий с deskjet |
| `initial_session` | войти один раз, сразу при старте greetd | `hosts/kitjet/default.nix` — только kitjet |

Поэтому tuigreet никуда не делся: он появится, если из сессии выйти.
А deskjet, который использует тот же `modules/desktop.nix`, автовход не
получает — там машина за столом, и это правильное место для разделения
«общее в `modules/`, специфика в `hosts/`».

Проверить, что доехало (файл генерируется в store, в `/etc/greetd/` его нет):

```bash
ssh root@kitjet.local 'cat $(systemctl cat greetd | grep -oE "/nix/store/\S+greetd\S+\.toml" | head -1)'
```

Должны быть обе секции — `[default_session]` с tuigreet и `[initial_session]`
с `niri-session`.

**Применяется со следующей загрузки.** `initial_session` срабатывает в момент
старта greetd, а при `switch` он не перезапускается — иначе выкинуло бы из
текущей сессии.

**Цена автовхода** — связка ключей gnome-keyring. Обычно её отпирает пароль,
введённый при входе; при автовходе вводить нечего, и связка останется
запертой до первого ручного ввода. Пока в ней ничего нет — не мешает, но
если однажды положишь туда ssh-ключ с паролем, вспомни про это ограничение,
см. [[SSH-ключи]].
...
[[00-Проект]] · [[Репозиторий]] · [[Бэкапы]] · [[Home-Assistant]] · [[ZFS-хранилище]] · [[Контейнеры]] · [[SSH-ключи]]
