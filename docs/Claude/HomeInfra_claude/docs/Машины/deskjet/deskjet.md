...
> **Статус: РАБОТАЕТ** (2026-09-13). NixOS установлен через kexec, `/home`
> с Arch подхватился целиком. Видео RDNA4 работает, вход по ssh-ключу
> настроен в обе стороны. Репозиторий склонирован и в `~/nixos` здесь —
> рабочих копий стало две, порядок работы с ними: [[Репозиторий]].
> Итоги установки — в разделе «Результат».
...
- [ ] Общий конфиг niri/DMS/ghostty для kitjet и deskjet — снимок в `dotfiles/`
