...
> **Проверено частично (2026-09-09).** Раздел про DSI писался до того, как
> выяснилось, что Pi 5 ставится только через `raspberry-pi-nix` — конфиг
> `hardware.raspberry-pi."5"` и `services.xserver` оттуда брать нельзя,
> актуальная схема в [[11-rpinix]]. Цифры потребления памяти и времени работы
> от батареи (600 МБ / 100 МБ, 1-2ч / 4-6ч) — прикидка, а не замер.
> Вывод «niri+DMS тяжелее Sway» верен качественно, числа нужно померить.
