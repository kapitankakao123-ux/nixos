...
> **ВНИМАНИЕ, раздел устарел (проверено 2026-09-09 на живой машине).**
> Пункты 2 и 3 ниже были верны для более старых nixpkgs, но **в 26.05 неверны**.
> Разбор — в [[13-rpinix-диагностика-загрузки]]. Оставлено, чтобы не искать заново.
...
2. ~~`ubootRaspberryPi5` не существует, значит U-Boot для Pi 5 нет.~~
   **Неверно.** Отдельного пакета действительно нет, но он и не нужен:
   `sd-image-aarch64` кладёт общий `u-boot.bin`, и он прекрасно грузит Pi 5.
   Проверено — live-система на SD загружена именно так, `config.txt` там
   начинается с `kernel=u-boot.bin`, ядро 7.2.4. Более того, в этом U-Boot
   скомпилирована поддержка NVMe (есть команда `nvme` и ссылки на PCIe).
3. ~~`sd-image-aarch64.nix` не знает про Pi 5.~~ **Неверно для 26.05.**
   В его `config.txt` есть секция `[pi5]`, а в раздел прошивки копируются
   `bcm2712-rpi-5-b.dtb`, `bcm2712-rpi-500.dtb` и все варианты CM5.
   В `[pi5]` апстрим ставит `enable_uart=0` с пояснением, что на части
   ревизий Pi 5 U-Boot ловит призрачный ввод с UART и прерывает загрузку
   (bugzilla.opensuse.org, баг 1251192).

Что осталось верным и стоило вечера:
...
Отсюда следовал переход на [`raspberry-pi-nix`](https://github.com/nix-community/raspberry-pi-nix).
Но раз стандартный образ на этой плате доказанно грузится, **этот выбор надо
пересмотреть**: вендорное ядро там запинено на nixpkgs января 2025, тянется
из отдельного кэша и уже стоило нам отдельного бага с `tpm-crb`.
...
Замирание на vc4-drm при загрузке — [[13-rpinix-диагностика-загрузки]].
